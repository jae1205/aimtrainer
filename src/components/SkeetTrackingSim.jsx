...
export default function SkeetTrackingSim({ onComplete, sensitivity, theme = 'dark', onStatsChange }) {
  ...
  return (
    ...
    >
      ...
      {completed && (
        <div className="absolute inset-0 z-30 flex items-center justify-center bg-black/70 backdrop-blur-sm">
          <div className={`text-center p-8 rounded-3xl border shadow-2xl max-w-sm w-full mx-4 ${panelCls}`}>
            <p className="text-xs font-semibold uppercase tracking-widest text-[#ff4655] mb-2">COMPLETE</p>
      ...
      {started && countdown === 0 && !completed && (
        <div className="absolute top-4 left-1/2 -translate-x-1/2 z-20 flex items-center gap-3">
          <div className="px-4 py-2 bg-black/50 rounded-xl backdrop-blur border border-white/10 text-white text-sm font-bold tabular-nums">
            {timeLeft}s
          </div>
          <div className="px-4 py-2 bg-black/50 rounded-xl backdrop-blur border border-[#22D3EE]/30 text-[#22D3EE] text-sm font-bold tabular-nums">
            {score} 파괴
          </div>
        </div>
      )}
